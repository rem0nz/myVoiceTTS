...
PREC = torch.bfloat16
OUT = Path("me_out"); OUT.mkdir(exist_ok=True)

REF_WAV, REF_TXT = Path("ref/me_clean.wav"), Path("ref/me_clean.txt")
...
CASES = {
    "me_neutral": LINE,
    "me_angry":   f"[super angry] {LINE}",
    "me_whisper": f"[whispers] {LINE}",
    "me_sad":     f"[very sad] {LINE}",
    "me_excited": f"[super excited] {LINE} [laugh]",
}
